...
import { FriendRequestStatus, FriendRequest_Status } from '../../services/models/FriendRequest';
    ...
    this.friendRequestStatusSubscription$ = this.getFriendRequestStatus()
      .pipe(
        tap((friendRequestStatus: FriendRequestStatus) => {
          ...
          console.log(3333, friendRequestStatus.status);
          this.userSubscription$ = this.getUser().subscribe((user: User) => {
            this.user = user;
            const imgPath = user.imagePath ?? 'blank-profile-picture-973460__340.webp';
            this.user['fullImagePath'] =
              'http://localhost:3000/api/v1/feed/images/' + imgPath;
          });
        })
      )
      .subscribe();
